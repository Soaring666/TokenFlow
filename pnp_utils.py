...
########conv add######
def register_cond_extract(model, t, save_path):
    def conv_forward(self):
        def forward(input_tensor, temb):
            ...
            ##########get and save feature from resnet####
            self.out_res_feature = hidden_states
            save_path = os.path.join(self.save_path, f"down_blocks2_resnets1_timestep_{self.t_step}.pt")
            torch.save(self.out_res_feature, save_path)
            ##############################################
    ...
    setattr(conv_module, 't_step', t)
    setattr(conv_module, 'save_path', save_path)
######################
