...
class PNP(nn.Module):
    def __init__(self, config):
        ...
        if sd_version == '2.1':
            model_key = "/home/flyvideo/PCH/diffusion/stable-diffusion-2-1-base"
        elif sd_version == '2.0':
        ...
        self.scheduler.set_timesteps(config["n_timesteps"], device=self.device)             #将内部的timesteps值从1000至0按步数均匀划分
    ...
    ############# extract downsample features from cond image ############
    @torch.no_grad()
    def run_extract_cond_features(self):
        save_path = self.config["output_path"]
        os.makedirs(save_path, exist_ok=True)
        #去噪生成
        with torch.autocast(device_type='cuda', dtype=torch.float32):
            for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="Sampling")):
                register_cond_extract(self, t, save_path)          #在原始模型中加入特征提取模块
                ...
                text_embed_input = self.pnp_guidance_embeds
                noise_pred = self.unet(source_latents, t, encoder_hidden_states=text_embed_input)['sample']
    ######################################################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str, default='configs/config_cond_extract.yaml')
